...
def normalize_date(date_input):
    ...
    logger.debug(f"normalize_date: {date_input} ({type(date_input)}) -> {result}")
    return result
...
def connect_to_schwab(token_path="schwab_token.json"):
    ...
    try:
        ...
        logger.error(f"❌ Error al inicializar Schwab Client: {e}", exc_info=True)
...
def obtener_datos_opcion(client, ticker, strike, tipo, fecha_salida):
    ...
    try:
        ...
        for idx, date_key in enumerate(sorted(option_map.keys()), 1):
        ...
        if not fecha_key_match:
            ...
            logger.error(f"Fechas disponibles:")
        ...
        for strike_key in strikes_dict.keys():
            try:
                ...
                logger.debug(f"  Strike: {strike_key} -> {strike_float}")
            except ValueError:
        ...
        logger.info(f"📋 Datos del contrato:")
        logger.info(f"  Keys: {list(contrato.keys())}")
        ...
        logger.error(f"❌ EXCEPCIÓN en obtener_datos_opcion: {e}", exc_info=True)
        logger.info("="*80)
        return None, None, None
    ...
    try:
        ...
        if symbol in quote_data:
            ...
            if 'quote' in ticker_data:
                ...
                logger.info(f"💰 Keys en quote: {list(quote.keys())}")
        ...
        logger.error(f"❌ EXCEPCIÓN en get_current_price_schwab: {e}", exc_info=True)
        logger.info("="*80)
        return None
...
def get_atm_strike_schwab(client, ticker, current_price, expiration_date):
    ...
    try:
        ...
        for map_type in ['callExpDateMap', 'putExpDateMap']:
            ...
            logger.info(f"\n📋 Fechas disponibles en {map_type} ({len(exp_map)}):")
            for idx, date_key in enumerate(sorted(exp_map.keys()), 1):
                logger.info(f"  {idx}. {date_key}")
            
            # Buscar la fecha exacta
            for date_key, strikes_dict in exp_map.items():
                if date_key.startswith(exp_date_str):
                    logger.info(f"\n✅ Fecha coincidente encontrada: {date_key}")
                    ...
                    for strike_key in strikes_dict.keys():
                        try:
                            ...
                            logger.debug(f"    Strike agregado: {strike_key} -> {strike_float}")
                        except ValueError:
                            logger.warning(f"    ⚠️ Strike inválido ignorado: {strike_key}")
                            continue
        ...
        if not available_strikes:
            ...
            for idx, d in enumerate(sorted(all_dates), 1):
        ...
        logger.info(f"\n📊 Strikes disponibles totales: {len(available_strikes_list)}")
        logger.info(f"  Rango: ${min(available_strikes_list):.2f} - ${max(available_strikes_list):.2f}")
        logger.info(f"  Primeros 5: {[f'${s:.2f}' for s in available_strikes_list[:5]]}")
        logger.info(f"  Últimos 5: {[f'${s:.2f}' for s in available_strikes_list[-5:]]}")
        ...
        logger.error(f"\n❌ EXCEPCIÓN en get_atm_strike_schwab: {e}", exc_info=True)
        logger.info("="*80 + "\n")
        return None
