...
# =========================================================================
# FUNCIONES TÉCNICAS SEMANALES
# =========================================================================

def calculate_donchian(df, period=20):
    """Calcula Canal de Donchian"""
...
def classify_regime_daily(df):
    """Clasifica régimen DIARIO usando ADX + RSI + SMAs"""
    regimes = []
...
def classify_regime_weekly(df):
    """Clasifica régimen SEMANAL usando Donchian + Choppiness + MACD-V"""
    regimes = []
...
def plot_daily_dashboard(df_recent, ticker):
    """Dashboard DIARIO con precio, RSI, ADX y timeline"""
    ...
    ax1.scatter(current.name, current['Close'], 
               facecolors='none',
               edgecolors=REGIME_COLORS_DAILY[current['Regime_Daily']], 
               s=450,
               linewidth=5,
               alpha=0.4,
               marker='o', 
               zorder=9)
    ax1.scatter(current.name, current['Close'], 
               facecolors=REGIME_COLORS_DAILY[current['Regime_Daily']],
               edgecolors='white', 
               s=280,
               linewidth=4,
               marker='o', 
               label=f'📍 Actual: {current["Regime_Daily"]}', 
               zorder=10)
    
    # Anotación régimen actual
    ...
    ax1.text(0.5, 1.05, 'Weekly Market Regime Analysis', 
            transform=ax1.transAxes,
    ...
    # GRÁFICO 2: CHOPPINESS INDEX
    ax2 = fig.add_subplot(gs[1], sharex=ax1)
    ax2.set_facecolor('#1A1D29')
    
    ax2.plot(df_recent.index, df_recent['Choppiness'], 
             color='#FFD93D', linewidth=2.5, label='Choppiness Index', zorder=3)
    
    ax2.fill_between(df_recent.index, df_recent['Choppiness'], 50,
                     where=(df_recent['Choppiness'] >= 61.8), 
                     color='#FFD93D', alpha=0.2, zorder=1)
    ax2.fill_between(df_recent.index, df_recent['Choppiness'], 50,
                     where=(df_recent['Choppiness'] <= 38.2), 
                     color='#4ECDC4', alpha=0.2, zorder=1)
    
    ax2.axhline(y=61.8, color='#FFD93D', linestyle='--', linewidth=2, alpha=0.8, zorder=2, label='Choppy > 61.8')
    ax2.axhline(y=50, color='#8E93A1', linestyle='-', linewidth=1.5, alpha=0.7, zorder=2)
    ax2.axhline(y=38.2, color='#4ECDC4', linestyle='--', linewidth=2, alpha=0.8, zorder=2, label='Trending < 38.2')
    ...
    ax2.scatter(current.name, current['Choppiness'], 
               facecolors='#FFD93D',
               edgecolors='white', 
               s=220,
               linewidth=4,
        ...
        if len(macd_signal) > 0:
            for i in range(1, len(df_recent)):
                if pd.notna(df_recent['MACD_V_Signal'].iloc[i-1]) and pd.notna(df_recent['MACD_V_Signal'].iloc[i]):
                    ...
                    ax3.plot([x1, x2], [y1, y2], color=color, linewidth=width, 
                            alpha=0.95, zorder=5)
    ...
    ax3.text(0.02, 0.88, f'MACD-V: {current["MACD_V_Signal"]:.1f}', 
...
def market_regime_page():
    ...
    with st.sidebar:
        ...
        lookback_months = st.slider(
            ...
            value=6,
            step=1,
            help="Selecciona el número de meses de historia a mostrar (mínimo 6 meses)"
        )
    ...
    if 'df_daily' in st.session_state and 'df_weekly' in st.session_state:
        ...
        <div style='text-align: center; margin-bottom: 20px;'>
            <h2 style='color: #00D9FF; font-size: 32px; margin: 0;'>📈 ANÁLISIS DIARIO</h2>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col1:
            regime_emoji = {
                ...
                'RANGO': '⚫',
                'RIESGO': '🟠'
            }[current_daily['Regime_Daily']]
            st.metric(
                "RÉGIMEN DIARIO",
                f"{regime_emoji} {current_daily['Regime_Daily']}"
            )
        ...
        with col6:
            st.metric(
                "FECHA",
                current_daily.name.strftime('%Y-%m-%d'),
                current_daily.name.strftime('%A')[:3]
            )
        ...
        st.markdown(f"""
        <div style='text-align: center; margin-bottom: 20px;'>
            <h2 style='color: #BD93F9; font-size: 28px;'>📊 Dashboard Técnico Diario</h2>
            <p style='color: #8E93A1; font-size: 16px;'>
                Últimos {lookback_months} meses • Timeframe: Daily (1d)
            </p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col1:
            ...
            st.metric(
                "RÉGIMEN SEMANAL",
                f"{regime_emoji} {current_weekly['Regime_Weekly']}"
            )
        
        with col2:
            st.metric(
                "PRECIO",
                f"${current_weekly['Close']:.2f}"
            )
        ...
        with col3:
            ...
            st.metric(
                "CHOPPINESS",
                f"{current_weekly['Choppiness']:.1f}",
                chop_status
            )
        ...
        with col4:
            ...
            st.metric(
                "MACD-V",
                f"{current_weekly['MACD_V_Signal']:.1f}",
                macd_status
            )
        ...
        with col6:
            st.metric(
                "FECHA",
                current_weekly.name.strftime('%Y-%m-%d'),
                "Semanal"
            )
        ...
        with col1:
            ...
            st.download_button(
                ...
                data=csv_daily,
                file_name=f"regime_daily_{ticker}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                mime="text/csv",
                use_container_width=True
            )
        
        with col2:
            export_cols_weekly = ['Close', 'Regime_Weekly', 'Choppiness', 'MACD_V_Signal', 
                                 'Donchian_Upper', 'Donchian_Middle', 'Donchian_Lower', 'SMA_20', 'SMA_50']
...
if __name__ == "__main__":
    if check_password():
        ...
        """, unsafe_allow_html=True), 'Daily Market Regime Analysis', 
            transform=ax1.transAxes,
            fontsize=13, style='italic',
            ha='center', color='#8E93A1')
...
def plot_weekly_dashboard(df_recent, ticker):
    """Dashboard SEMANAL con precio, Donchian, Choppiness, MACD-V y timeline"""
    
    plt.style.use('dark_background')
    fig = plt.figure(figsize=(24, 14), facecolor='#0E1117')
    gs = fig.add_gridspec(4, 1, height_ratios=[3.5, 1, 1, 1], hspace=0.4)
    
    # GRÁFICO 1: PRECIO CON DONCHIAN Y REGÍMENES
    ax1 = fig.add_subplot(gs[0])
    ax1.set_facecolor('#1A1D29')
    
    # Donchian Channel
    ax1.fill_between(df_recent.index, 
                     df_recent['Donchian_Upper'], 
                     df_recent['Donchian_Lower'],
                     alpha=0.08, color='#00D9FF', zorder=0)
    
    ax1.plot(df_recent.index, df_recent['Donchian_Upper'], 
             color='#00D9FF', alpha=0.6, linewidth=2, linestyle='--',
             label='Donchian Upper', zorder=2)
    ax1.plot(df_recent.index, df_recent['Donchian_Middle'], 
             color='#00D9FF', alpha=0.8, linewidth=2.5, linestyle='-',
             label='Donchian Middle', zorder=2)
    ax1.plot(df_recent.index, df_recent['Donchian_Lower'], 
             color='#00D9FF', alpha=0.6, linewidth=2, linestyle='--',
             label='Donchian Lower', zorder=2)
    ...
    # SMAs semanales
    ax1.plot(df_recent.index, df_recent['SMA_20'], 
             color='#50FA7B', alpha=0.9, linewidth=2.5, linestyle='-',
             label='SMA(20) Weekly', zorder=3)
    ax1.plot(df_recent.index, df_recent['SMA_50'], 
             color='#BD93F9', alpha=0.9, linewidth=2.5, linestyle='-',
             label='SMA(50) Weekly', zorder=3)
    
    # Punto actual destacado
    current = df_recent.iloc[-1]
    ax1.scatter(current.name, current['Close'], 
               facecolors='none',
               edgecolors=REGIME_COLORS_WEEKLY[current['Regime_Weekly']], 
               s=450,
               linewidth=5,
               alpha=0.4,
               marker='o', 
               zorder=9)
    ax1.scatter(current.name, current['Close'], 
               facecolors=REGIME_COLORS_WEEKLY[current['Regime_Weekly']],
               edgecolors='white', 
               s=280,
               linewidth=4,
               marker='o', 
               label=f'📍 Actual: {current["Regime_Weekly"]}', 
               zorder=10)
