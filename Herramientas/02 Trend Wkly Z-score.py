...
def classify_regime_zscore(df):
    """Clasifica el régimen de mercado con 7 estados"""
    regimes = []
    ...
    for idx, row in df.iterrows():
        ...
        if trend_up:
            ...
            elif 1.5 < z <= 2.0 and momentum_bullish:
                if not pd.isna(macd_v_diff) and macd_v_diff <= 0:
                    regime = 'SOBRECOMPRA_DEBIL'
                else:
                    regime = 'ALCISTA'
            ...
            elif -2.0 <= z < -1.5 and momentum_bearish:
                if not pd.isna(macd_v_diff) and macd_v_diff >= 0:
                    regime = 'SOBREVENTA_DEBIL'
                else:
                    regime = 'BAJISTA'
...
def plot_zscore_dashboard(df_recent, ticker):
    """Crea el dashboard con 6 gráficos"""
    plt.style.use('dark_background')
    fig = plt.figure(figsize=(24, 22), facecolor='#0E1117')
    gs = fig.add_gridspec(6, 1, height_ratios=[3.5, 1.2, 1.2, 1, 1, 2.2], hspace=0.55)
    ...
    ax1.text(0.5, 1.05, 'Z-Score MACD-V Market Regime Analysis (20w) - v3.0',
    ...
    ax5.set_xlabel('Date', fontsize=13, fontweight='700', color='#FFFFFF')
    
    # GRÁFICO 6: DOCUMENTACIÓN
...
╠══════════════════════════════════════════════════════════════════════════════════════════════════════════════════╣
║ VARIABLES:                                                                                                       ║
║  • Z-Score Adjusted: momentum MACD-V normalizado y ajustado por curtosis (-4σ a +4σ)                            ║
║  • MACD-V: MACD / ATR × 100. MACD-V > Signal = momentum alcista                                                  ║
║  • SMA(50): precio > SMA50 = contexto alcista; precio < SMA50 = contexto bajista                                ║
║  • ΔMACD-V: dirección del MACD-V → separa Fuerte (acelera) vs Débil (desacelera)                                ║
║                                                                                                                  ║
...
║  🔴 SOBRECOMPRA_DEBIL: (Z > 2.0σ O 1.5σ < Z ≤ 2.0σ) + MACD-V > Signal + ΔMACD-V ≤ 0                             ║
║     → Extremo alcista pero momentum se debilita. Corrección próxima.                                            ║
...
║  🟣 SOBREVENTA_DEBIL: (Z < -2.0σ O -2.0σ ≤ Z < -1.5σ) + MACD-V < Signal + ΔMACD-V ≥ 0                           ║
...
╚══════════════════════════════════════════════════════════════════════════════════════════════════════════════════╝
v3.0: Zonas de transición 1.5σ-2.0σ y -2.0σ/-1.5σ detectan DÉBIL antes de salir de extremos.
    """
    
    ax6.text(0.5, 0.5, doc_text, transform=ax6.transAxes, fontsize=8.5, verticalalignment='center',
...
def zscore_analyzer_page():
    ...
    st.markdown("### *Statistical Momentum with Kurtosis Adjustment (20w) - v3.0*")
    st.markdown("---")
