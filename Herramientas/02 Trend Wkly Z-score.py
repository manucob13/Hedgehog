...
def classify_by_zscore(df):
    ...
    for i in range(len(df)):
        ...
        z_price_extreme = abs(z_price) > 2.5
        ...
        if z_price_extreme:
            ...
            if -0.5 <= z_price <= 0.5:
                ...
                regime = "ALCISTA" if z_price > 0.5 else "RANGO"
            else:
                regime = "BAJISTA" if z_price < -0.5 else "RANGO"
...
def analyze_risk(df):
    ...
    for i in range(len(df)):
        ...
        risk_levels.append("RIESGO+" if (macd_extreme and z_extreme) else "Sin Riesgo")
...
def download_weekly_data(ticker, years_back):
    ...
    # Asegurar que las columnas sean simples strings (por si acaso)
