...
def classify_regime_zscore(df):
    ...
    for idx, row in df.iterrows():
        ...
        if price > sma50:
            if z > 1.5:
                # Transición obligatoria: Solo FUERTE si acelera y Z > 2
...
st.markdown("### *Statistical Momentum with Transition Logic v3.1*")
...
if 'df' in st.session_state:
    ...
    fig = plt.figure(figsize=(20, 18), facecolor='#0E1117')
    gs = fig.add_gridspec(4, 1, height_ratios=[3, 1, 1, 1.5], hspace=0.3)

    # 1. Gráfico de Precio
    ax1 = fig.add_subplot(gs[0])
    ax1.set_facecolor('#1A1D29')
    ax1.plot(df_recent.index, df_recent['Close'], color='white', alpha=0.5)
    ...
    for regime, color in REGIME_COLORS_ZSCORE.items():
        ...
        if mask.any():
            ax1.scatter(df_recent[mask].index, df_recent[mask]['Close'], c=color, label=regime, s=80, edgecolors='white', linewidth=0.5)
    ax1.legend(loc='upper left', ncol=3)
    ax1.set_title(f"Análisis {tk}", fontsize=20)
    ...
    ax2.axhline(-1.5, color='purple', linestyle=':', alpha=0.5)
    ax2.axhline(-2.0, color='purple', linestyle='--')
    ax2.fill_between(df_recent.index, 0.75, -0.75, color='yellow', alpha=0.05)
    ax2.set_ylabel("Z-Score Adjusted")
    ...
    # 4. Timeline de Régimen
    ax4 = fig.add_subplot(gs[3], sharex=ax1)
    ax4.set_facecolor('#1A1D29')
    order = list(REGIME_COLORS_ZSCORE.keys())
    y_vals = [order.index(r) for r in df_recent['Regime_ZScore']]
    ax4.scatter(df_recent.index, y_vals, c=[REGIME_COLORS_ZSCORE[r] for r in df_recent['Regime_ZScore']], s=120, marker='s')
