...
Basado en el mismo esqueleto que "CC ATM Screener" (ETFs laterales), pero
adaptado por completo a la estrategia ITM Covered Call definida:
...
RISK_FREE_RATE = 0.045  # aproximación; ajustar si se desea usar la tasa vigente
...
def calculate_sma_trend(close, period=30, slope_lookback=5):
    """
    Retorna (sma_actual, distancia_%_sobre_sma, sma_slope_positiva)
    - sma_slope_positiva: True si SMA30 de hoy > SMA30 de hace `slope_lookback` días
      (confirma que la media está subiendo, no solo que el precio la cruzó)
    """
    try:
...
def calculate_realized_vol(close, window=10):
    """Volatilidad realizada anualizada (%) sobre los últimos `window` días."""
    try:
...
def get_next_earnings_date(stock):
    """Fecha del próximo earnings (o None)."""
    try:
        cal = stock.calendar
...
def get_next_ex_dividend_date(stock):
    """Fecha del próximo ex-dividend (estimado si no está publicado)."""
    try:
        cal = stock.calendar
...
# ======================================================================
# 5. SELECCIÓN DE VENCIMIENTO (viernes a viernes, ~ target_dte días calendario)
# ======================================================================

def select_target_expiration(expirations, target_dte_days=7, tolerance_days=3):
    """
    Elige la expiración disponible más cercana a `target_dte_days` días
    calendario desde hoy, dentro de una tolerancia. Retorna (exp_str, dte_calendar) o (None, None).
    """
    try:
        today = date.today()
        best = None
...
# ======================================================================
# 6. BLACK-SCHOLES: DELTA DE LA CALL (yfinance no trae griegos)
# ======================================================================

def bs_call_delta(S, K, T_years, sigma, r=RISK_FREE_RATE):
    """Delta de una call europea via Black-Scholes. T_years en años, sigma en decimal."""
    try:
...
def find_itm_candidate(calls_df, current_price, dte_calendar,
                        ...
                        delta_min, delta_max):
    """
    Recorre los strikes ITM (strike < current_price) de la cadena de calls
    y devuelve el mejor candidato que cumple extrínseco% y delta objetivo.
    Si hay varios, prioriza el extrínseco% más cercano al techo del rango.
    """
    try:
        ...
        # Prioriza extrínseco% más alto dentro del rango (mejor yield del ciclo)
...
# ======================================================================
# 8. PUT/CALL RATIO (Yahoo derivado, con CBOE oficial como preferencia)
# ======================================================================

def get_pcr_yahoo(stock, exp_str, current_price, price_range_pct=10):
    """PCR derivado de la cadena de yfinance en el vencimiento objetivo (fallback)."""
    try:
...
def get_pcr_cboe(ticker, exp_str):
    """PCR oficial desde CBOE (volumen), restringido al vencimiento objetivo."""
...
def _minmax_score(value, lo, hi, invert=False):
    """Normaliza value a 0-100 dentro de [lo, hi]. invert=True: menor es mejor."""
...
# ======================================================================
# 10. ANÁLISIS DE UN TICKER (todos los filtros de elegibilidad + métricas)
# ======================================================================
...
def analyze_ticker_itm(ticker, params):
    try:
        ...
        # --- Volatilidad realizada (para IV/RV) ---
        ...
        # --- Filtro F: earnings dentro de la ventana (+2 días de colchón) ---
        ...
        # --- Filtro F: ex-dividend dentro de la ventana ---
        ...
        # --- Filtros D/E: extrínseco% + delta -> mejor strike ITM ---
        ...
        # --- Filtro B: liquidez de la cadena en el strike elegido ---
        ...
        if strike_volume < params['min_option_volume'] and strike_oi < params['min_oi'] * 1.5:
            # Si no hay volumen del día, exige más colchón de OI para compensar
            return None
