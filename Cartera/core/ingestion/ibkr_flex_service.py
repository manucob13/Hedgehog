...
def _fetch_statement(
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        # Si la raíz es <FlexQueryResponse>, el reporte está listo.
        if resp.text.lstrip().startswith("<FlexQueryResponse"):
            return resp.text

        # Si no, es un <FlexStatementResponse> de error o "todavía no está listo".
        root = ET.fromstring(resp.text)
        error_code = root.findtext("ErrorCode", default="???")
        error_message = root.findtext("ErrorMessage", default="Error desconocido")

        if error_code == "1019":  # generación en curso, reintentar
...
def fetch_flex_report(token: str, query_id: str) -> str:
    ...
    Devuelve el XML crudo (texto), listo para pasar a flex_query.import_flex_report()
    (o para guardarlo primero en disco con save_raw_xml()).
    """
